...
export default function puzzle(
  ...
  for (let maxByte = startingMaxBytes; maxByte < bytePositions.length; maxByte += 1) {
    const map: (Cell | null)[][] = [];

    for (let y = 0; y < mapHeight; y += 1) {
      map[y] = [];
      for (let x = 0; x < mapWidth; x += 1) {
        for (let n = 0; n <= maxByte; n += 1) {
    ...
    if (!result.length) {
      return `${bytePositions[maxByte].x},${bytePositions[maxByte].y}`;
    }
  }
